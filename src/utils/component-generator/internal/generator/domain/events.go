...
func GenerateEventFiles(config model.ComponentConfig, basePath string) {
	...
	deletedEventContent.WriteString(fmt.Sprintf("import { Deleted%sEntity } from '../entities/%s.entity'\n\n", singularCap, singular))
	...
	deletedEventContent.WriteString(fmt.Sprintf("    public readonly %s: Deleted%sEntity\n\n", singular, singularCap))
	deletedEventContent.WriteString(fmt.Sprintf("    constructor(%s: Deleted%sEntity) {\n", singular, singularCap))
