...
import { OracleDbService } from 'src/api/v1/infrastructure/oracle/oracledb' 
...
export class OracleTaskDatasource implements TaskDatasource {
    public async getTasks(email: string, status: string): Promise<TaskEntity[]> {
        const response: QueryResult = await OracleDbService.query(`SELECT * FROM ${envs.dbSchema}."Task"`)    
    ...
    public async createTask(name: string, search: string, userId: number): Promise<TaskEntity> {
        ...
        const response: QueryResult = await OracleDbService.query(`INSERT INTO ${envs.dbSchema}."Task"(
