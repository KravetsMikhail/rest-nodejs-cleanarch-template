...
|----------|-----|----------|--------|
| `offset` | integer | Смещение от начала списка | `?offset=10` |
| `limit` | integer | Количество записей на странице | `?limit=20` |
...
Фильтры задаются через query-параметры. Поддерживаются операторы сравнения через суффиксы:

| Суффикс | SQL оператор | Описание | Пример |
|---------|--------------|----------|--------|
| (без суффикса) | `=` | Точное совпадение | `?status=DRAFT` |
| `_like` | `ILIKE '%...%'` | Поиск подстроки (регистронезависимый) | `?name_like=задача` |
| `_gte` | `>=` | Больше или равно | `?createdAt_gte=2024-01-01` |
| `_lte` | `<=` | Меньше или равно | `?price_lte=1000` |
| `_gt` | `>` | Строго больше | `?age_gt=18` |
| `_lt` | `<` | Строго меньше | `?date_lt=2024-12-31` |
| `_ne` | `<>` | Не равно | `?status_ne=CANCELED` |
| `_contains` | `@>` (JSONB) | JSON содержит значение | `?metadata_contains={"active":true}` |
| `_key` | `->>'key'` | Поиск по ключу в JSON (формат: `key:value`) | `?config_key=theme:dark` |

#### Поддерживаемые типы данных

| Тип | Поведение | Примеры операторов |
|-----|-----------|-------------------|
| **String** | `ILIKE` для `_like`, точное совпадение для остальных | `_like`, `=`, `_ne` |
| **Number** | Числовое сравнение | `=`, `_gte`, `_lte`, `_gt`, `_lt`, `_ne` |
| **BigInt** | Аналогично Number | `=`, `_gte`, `_lte`, `_gt`, `_lt`, `_ne` |
| **ID** | Числовое сравнение (для идентификаторов) | `=`, `_gte`, `_lte`, `_gt`, `_lt`, `_ne` |
| **Boolean** | Преобразование `true/false` в SQL | `=` |
| **Date** | Автоформатирование в ISO, сравнение дат | `=`, `_gte`, `_lte`, `_gt`, `_lt` |
| **JSON** | JSONB операторы PostgreSQL | `_contains`, `_key`, `_like` |
| **Array** | Проверка вхождения в массив (`= ANY()`) | `=` |
...
GET /settings?config_key=language:ru
```
